...
void setup() {
    ...
    // --- Initialize GPIO ---
    pinMode(kSDCPin, INPUT_PULLDOWN); // Changed to PULLDOWN for active high
...
void printSystemStatus() {
    ...
    BMS_STATES current_bms_state = BMS_dev.state; //
    CHARGING_STATES current_charging_state = BMS_dev.charging_state; //
    ...
    // Print details only if BMS state is valid
     if (current_bms_state != BMS_SIGNAL_THRESHOLD) { //
    ...
    bool anyFault = false;
    if (digitalRead(kSDCPin) == HIGH) { // Active HIGH fault
        Serial.print("[SDC] ");
        anyFault = true;
    }
...
void updateChargingState() {
    ...
    // Debug: Print current state and conditions periodically
    static uint32_t lastDebugMs = 0;
    if (nowMs - lastDebugMs > 2000) { // Every 2 seconds
        Serial.printf("DEBUG: State=%d, BMS_state=%d, BatteryOk=%d, SDC=%d, Button=%d, OBC_U=%.1f, HndlSt=%d, ChrgrSt=%d\n",
                      BMS_dev.charging_state, BMS_dev.state, isBatteryHealthy(), digitalRead(kSDCPin), digitalRead(kStartButtonPin),
                      obc.onBdChrgrUDc, obc.onBdChrgrHndlSt, obc.onBdChrgrSt);
        lastDebugMs = nowMs;
    }
    ...
    switch (currentState) {
        ...
        case CHG_READY:
            ...
            if (buttonPressed) {
                 if (!digitalRead(kSDCPin)) { // LOW = normal
                    BMS_dev.charging_state = CHG_START;
                    Serial.println("STATE: -> START (Button Pressed)");
                 } else {
                     // Button pressed but SDC open -> Go to FAIL
                     BMS_dev.charging_state = CHG_FAIL;
                     Serial.println("STATE: -> FAIL (Button pressed, but SDC open)");
                 }
            }
            // Transition back to FAIL if SDC becomes open while waiting
            if (digitalRead(kSDCPin)) { // HIGH = fault
                BMS_dev.charging_state = CHG_FAIL;
